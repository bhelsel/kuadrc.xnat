...
save_nifti_image <- function(path = NULL, image_type = "png", 
  ...
  if(grepl("nii.gz$", path)){
    ...
    files <- list.files(path, recursive = TRUE, full.names = TRUE, pattern = "nii.gz$")
  }
  
  directory <- strsplit(x = files[1], "/nifti")[[1]][1]
  if(!dir.exists(file.path(directory, "images"))) dir.create(file.path(directory, "images"))
  image_files <- gsub("nifti", "images", gsub("nii.gz$", image_type, files))
  
  for(i in 1:length(image_files)){
    if(plane == "") pl <- guess_view(files)[i]
    if(!dir.exists(dirname(image_files[i]))) dir.create(dirname(image_files[i]))
    data <- read_nifti(files[i])
    p <- plot_nifti(data, plane = pl, ...)
    ggplot2::ggsave(
      filename = image_files[i], plot = p, 
