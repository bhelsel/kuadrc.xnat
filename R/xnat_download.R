...
xnat_download <- function(outdir, ...) {
  ...
  if (is.null(params$subject) & is.null(params$experiment)) {
    subject_no <-
      utils::menu(
        ...
        choices = subjects$label
      )
    params$subject <- subjects[subject_no, "label"]
  }
  ...
  params$experimentLabel <- experiments[
    which(experiments$ID == params$experiment),
    "label"
  ]
  ...
  if (!is.null(params$scan)) {
    ...
    if ("all" %in% tolower(params$scan)) {
      ...
      if (length(params$scan) > 1) {
        params$scan <- paste0(
          grep(paste0(params$scan, collapse = "|"), scans$type),
          collapse = ","
        )
      }
